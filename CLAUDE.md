...
- **Reporting:** Post-execution audits generate `winauto_audit.json` in `$Global:WinAutoLogDir`, which resolves to `%USERPROFILE%\logs` under normal execution. `$PWD.Path` is a last-resort fallback only (reached when the log path's parent directory can't be determined) — not the primary intent. Confirmation messages should be indented (4 spaces) and Cyan-colored.
