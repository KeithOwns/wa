...
<#
.SYNOPSIS
    Enables 'App & browser control' in Windows Security via UI Automation.
.DESCRIPTION
    Launches Windows Security and attempts to click the 'Turn on' button
    associated with 'App & browser control'.
#>
...
Write-Log "Starting Windows Security Automation (App & browser)..." "Cyan"
...
while ($KeepLooping -and ($RetryCount -lt $MaxRetries)) {
    ...
    Write-Log "Searching for 'App & browser control' section..." "Gray"
    $Section = Get-UIAElement -Parent $Window -Name "App & browser control" -Scope "Descendants" -TimeoutSeconds 5

    if ($Section) {
        Write-Log "Found 'App & browser control' section." "Green"
        ...
        Write-Log "Could not find 'App & browser control' section." "Red"
    }
}
