...
function Set-WinAutoLastRun {
    ...
    try {
        ...
        Write-Log "Failed to update LastRun for $Module -Level WARN
    }
}
...
#>
    ...
    try {
        ...
        $statusText = if ($Reverse) { "ENABLED" } else { "ENABLED" }
...
while ($true) {
    ...
    if ($MenuSelection -eq 2) {
        Add-DashLine "  ${Global:FGBlack}${Global:BGYellow}___________________MANUAL MODE_________________${Global:Reset}"
    }
    else {
        Add-DashLine "  ${Global:FGBlack}${Global:BGDarkGray}_________________MANUAL-MODE-OFF_______________${Global:Reset}"
    }
