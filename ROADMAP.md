...
- [ ] **Propagate UIA foreground fix + verify all UIA steps (PRIORITY):** Only Telemetry and Phishing have the `Set-WinAutoForeground` fix so far. Add it to the remaining UIA steps and verify each end-to-end with the self-test method above. **Metered Updates is most urgent** — its polarity was flipped (forward = ON) and committed, but it still lacks the foreground fix, so that behavior change is currently UNVERIFIED and may still silently no-op when run. Remaining steps needing the fix: Metered Updates, SmartScreen-UIA (`Invoke-WA_SetSmartScreen`), VirusThreatProtect-UIA (`Invoke-WA_SetVirusThreatProtect`), KernelMode-UIA (`Invoke-WA_SetKernelMode`), RealTimeProtUI (`SET_RealTimeProtUI.ps1`). Deliverable: a pass/fail table proving every UIA step actually drives its control. (ARSO already works via incidental `SendKeys` focus, but should get the explicit helper for consistency.)
