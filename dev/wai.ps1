...
<#
.SYNOPSIS
    WinAuto (App Integrated Edition)
.DESCRIPTION
...
Install Applications     ${FGDarkBlue}|${Reset} ${FGYellow}Instll${Reset}   ${FGDarkBlue}|${Reset} ${FGGray}INLINED via wai.ps1${Reset}
...
Install,Install Applications,Mixed,Iterates through INLINED apps list,No,No,System,wai.ps1,Invoke-WA_InstallApps
...
function Get-WA_InstallAppList {
    # Hard-coded application list for standalone execution (wai.ps1)
    $config = @{
        BaseApps   = @(
            @{ 
    ...
    $AppList = @()
    if ($config.BaseApps) { 
        foreach ($app in $config.BaseApps) { $AppList += [PSCustomObject]$app }
    }
    if (-not $IsDesktop -and $config.LaptopApps) { 
        foreach ($app in $config.LaptopApps) { $AppList += [PSCustomObject]$app }
    }
...
function Invoke-WA_InstallApps {
    ...
    if (-not $AppList -or $AppList.Count -eq 0) {
        Write-LeftAligned "$FGDarkYellow$Global:Char_Warn No applications defined for installation.$Reset"
        return
    }
