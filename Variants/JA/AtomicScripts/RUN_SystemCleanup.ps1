...
& {
    ...
    $Reset = "$Esc[0m"
    $Bold = "$Esc[1m"
    $FGGreen = "$Esc[92m"; $FGRed = "$Esc[91m"; $FGCyan = "$Esc[96m"; $FGDarkBlue = "$Esc[34m"; $FGWhite = "$Esc[97m"; $FGYellow = "$Esc[93m"; $FGGray = "$Esc[37m"

    $Char_HeavyCheck = "[v]"; $Char_HeavyMinus = "-"; $Char_Warn = "!"; $Char_RedCross = "[x]"
            ...
            if (Test-Path $p) {
                ...
                try {
                    ...
                    if ($items) {
                        ...
                        Write-LeftAligned "  $FGGreen[v] Removed $c items.$Reset"
                        $total += $c
                    }
                    else { Write-LeftAligned "  $FGGray Already empty.$Reset" }
                }
                catch { Write-LeftAligned "  $FGRed$Char_Warn Partial cleanup failure.$Reset" }
            }
        }
        Write-Host ""; Write-LeftAligned "$FGGreen$Char_HeavyCheck Cleanup Complete. Total items removed: $total$Reset"
    }
